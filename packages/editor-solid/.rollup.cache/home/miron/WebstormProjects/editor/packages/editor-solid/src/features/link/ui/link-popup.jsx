...
const LinkPopupInner = () => {
    ...
    const { instance, input, selection, href, setHref, hadHref } = useLinkPopupState();
    ...
    return ReactDOM.createPortal(<form onSubmit={handleSubmit}>
      <InputWrapper>
        <LinkIcon>
          <LinkIconSvg />
        </LinkIcon>
        <Input ref={input} value={href} onChange={handleChange} placeholder="Enter the URL"/>
        {hadHref.current && (<ClearIcon onClick={clear}>
            <ClearIconSvg />
          </ClearIcon>)}
      </InputWrapper>
...
//# sourceMappingURL=link-popup.jsx.map