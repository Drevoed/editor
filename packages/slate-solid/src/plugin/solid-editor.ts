...
/**
 * A Solid and DOM-specific version of the `Editor` interface.
 */
...
export const SolidEditor = {
  ...
  hasDOMNode(
    ...
    const editorElement = SolidEditor.toDOMNode(editor, editor)
    let targetElement
    ...
    return (
      targetElement.closest(`[data-slate-editor]`) === editorElement &&
      (!editable || targetElement.isContentEditable
        ? true
        : (typeof targetElement.isContentEditable === 'boolean' && // isContentEditable exists only on HTMLElement, and on other nodes it will be undefined
            // this is the core logic that lets you know you got the right editor.selection instead of null when editor is contenteditable="false"(readOnly)
            targetElement.closest('[contenteditable="false"]') ===
              editorElement) ||
          Boolean(targetElement.getAttribute('data-slate-zero-width')))
    )
  },
  ...
  toDOMPoint(editor: SolidEditor, point: Point): DOMPoint {
    ...
    const element = SolidEditor.toDOMNode(editor, node)
  ...
  toDOMRange(editor: SolidEditor, range: Range): DOMRange {
    ...
    const isStartAtZeroWidth = Boolean(
      startElement.getAttribute('data-slate-zero-width')
    )
    const endElement = (
      isDOMElement(endNode) ? endNode : endNode.parentElement
    ) as HTMLElement
    const isEndAtZeroWidth = Boolean(
      endElement.getAttribute('data-slate-zero-width')
    )
  ...
  toSlateNode(editor: SolidEditor, domNode: DOMNode): Node {
    let domElement = isDOMElement(domNode) ? domNode : domNode.parentElement

    if (domElement && !domElement.hasAttribute('data-slate-node')) {
      domElement = domElement.closest(`[data-slate-node]`)
    }

    const node = domElement
      ? ELEMENT_TO_NODE.get(domElement as HTMLElement)
      : null

    if (!node) {
      throw new Error(
        `Cannot resolve a Slate node from DOM node: ${domElement}`
      )
    }
  ...
  toSlateRange<T extends boolean>(
    ...
    const { exactMatch, suppressThrow } = options
    const element = isDOMSelection(domRange)
      ? domRange.anchorNode
