...
export const withSolid = <T extends Editor>(editor: T) => {
    ...
    switch (op.type) {
      ...
      case 'set_node': {
        for (const [node, path] of Editor.levels(e, { at: op.path })) {
          const key = SolidEditor.findKey(e, node)
  ...
  e.setFragmentData = (data: Pick<DataTransfer, 'getData' | 'setData'>) => {
    ...
    const domRange = SolidEditor.toDOMRange(e, selection)
    ...
    if (endVoid) {
      ...
      const domNode = SolidEditor.toDOMNode(e, voidNode)
